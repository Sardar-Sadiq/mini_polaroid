...
import { Camera, Download, Trash2, Sparkles, RefreshCw, Smartphone, Film, Frame, Moon, Clock, Monitor, Wind, Ghost, Zap, Cloud, Image as ImageIcon, Smile, Clapperboard, FlipHorizontal } from 'lucide-react';
...
const App = () => {
  ...
  // Detect mobile version based on screen width
  useEffect(() => {
    const checkMobile = () => {
  ...
  const executeCapture = useCallback(() => {
    ...
    const padding = imgSize * 0.05; 
    const bottomPadding = imgSize * 0.20; 
    ...
    // Only mirror the final photo if using the front camera
    if (facingMode === 'user') {
      context.scale(-1, 1);
    } else {
      context.translate(-imgSize, 0);
      context.scale(1, 1);
    }
  ...
  }, [colorFilter, activeFrame, facingMode, saveFinal]);

  // Restart Camera whenever facingMode changes
  useEffect(() => {
    const startCamera = async () => {
      // Stop previous tracks to free up hardware
  ...
  return (
    <div className="min-h-screen bg-[#f3f0e9] font-serif text-slate-800 flex flex-col md:flex-row overflow-hidden">
      ...
      <div className="w-full md:w-1/3 h-full md:h-screen bg-[#e8e4d9] border-r-4 border-[#dcd7c9] p-6 overflow-y-auto custom-scrollbar shadow-inner">
        <div className="flex items-center gap-3 pb-4 mb-8 border-b-2 border-slate-300">
          <Smartphone className="text-slate-500" />
          <h1 className="text-2xl italic font-bold tracking-tight uppercase text-slate-700">Booth Gallery</h1>
        </div>
        ...
        {captures.length === 0 ? (
          ...
          <div className="grid grid-cols-1 gap-12 pb-10">
            {captures.map((cap) => (
              <div key={cap.id} className="relative transition-all duration-300 transform shadow-2xl group rotate-1 hover:rotate-0">
                ...
                <div className="absolute flex gap-2 transition-opacity opacity-0 top-4 right-4 group-hover:opacity-100">
                  <button onClick={() => downloadImage(cap.url, cap.id)} className="p-2 text-white bg-blue-500 rounded-full shadow-lg hover:bg-blue-600">
                    <Download size={18} />
                  </button>
                  <button onClick={() => removeImage(cap.id)} className="p-2 text-white bg-red-500 rounded-full shadow-lg hover:bg-red-600">
                    <Trash2 size={18} />
                  </button>
                </div>
              </div>
      ...
      <div className="flex-1 h-screen flex flex-col p-4 md:p-8 items-center justify-center relative bg-[#f9f7f2]">
        ...
        <div className="relative w-full max-w-sm md:max-w-md bg-white p-3 pb-20 shadow-[0_20px_50px_rgba(0,0,0,0.15)] border border-slate-200">
          ...
          <div className="relative flex items-center justify-center overflow-hidden bg-black aspect-square">
            {/* Flip Camera Button (Only visible on Mobile) */}
            {isMobile && (
              <button 
                onClick={toggleFacingMode}
                className="absolute top-4 right-4 z-[60] bg-black/40 hover:bg-black/60 text-white p-2 rounded-full backdrop-blur-md border border-white/20 transition-all"
                title="Switch Camera"
              >
            ...
            <video 
              ...
              playsInline 
              muted
              className={`w-full h-full object-cover transition-all duration-300 ${facingMode === 'user' ? 'scale-x-[-1]' : 'scale-x-[1]'} ${colorFilterClasses[colorFilter]}`}
            />
            ...
            {activeFrame === 'chill' && (
              <div className="absolute text-3xl italic text-white pointer-events-none bottom-6 right-6 drop-shadow-lg opacity-90 font-handwriting">chill homie!</div>
            )}
            ...
            {isCapturing && <div className="absolute inset-0 z-50 bg-white animate-ping" />}
          </div>

          <div className="absolute flex flex-col bottom-4 left-6 text-slate-400">
            <span className="text-lg font-handwriting">{new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span>
        ...
        <div className="w-full max-w-lg mt-8 space-y-6">
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap justify-center gap-2 p-2 overflow-y-auto max-h-32 custom-scrollbar">
              {[
          ...
          <div className="flex items-center justify-center gap-6">
            <div className="flex flex-col items-center gap-1">
              <div className="flex p-1 bg-white border-2 rounded-full border-slate-200">
                {[0, 3, 5].map((t) => (
            ...
            <button onClick={handleCaptureClick} disabled={isCapturing || countdown !== null} className="relative flex items-center justify-center w-20 h-20 transition-all bg-red-500 border-8 border-white rounded-full shadow-2xl group hover:bg-red-600 active:scale-95 disabled:opacity-50">
              <div className="absolute inset-0 border-4 rounded-full border-slate-800 opacity-20" />
              <Camera className="text-white transition-transform group-hover:rotate-12" size={28} />
            </button>
            
            <div className="flex flex-col items-center gap-1">
              <div className="flex p-1 bg-white border-2 rounded-full border-slate-200">
                <button onClick={() => setActiveFrame(activeFrame === 'cinema' ? 'none' : 'cinema')} className={`w-10 h-10 rounded-full flex items-center justify-center text-[10px] font-bold ${activeFrame === 'cinema' ? 'bg-slate-800 text-white' : 'text-slate-400'}`}><Clapperboard size={14}/></button>
                <button onClick={() => setActiveFrame(activeFrame === 'chill' ? 'none' : 'chill')} className={`w-10 h-10 rounded-full flex items-center justify-center text-[10px] font-bold ${activeFrame === 'chill' ? 'bg-slate-800 text-white' : 'text-slate-400'}`}><Smile size={14}/></button>
              </div>
